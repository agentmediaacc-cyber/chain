...
def _supabase_auth_email_exists(email):
    ...
    if not normalized:
        return False
    try:
        users = get_supabase_admin().auth.admin.list_users()
        for user in users:
            if clean_email(getattr(user, "email", None)) == normalized:
                return True
    except Exception as error:
        print(f"[auth_service] auth user email lookup failed: {error}")
    return False
...
def check_account_availability(field, value, town=None):
    ...
    if field == "username":
        ...
        return {
            ...
            "suggestions": [] if not taken else username_suggestions(normalized, town=town),
        }
    ...
    if field == "email":
        ...
        return {
            "available": not taken,
            "field": field,
            "message": "Email is available." if not taken else "That email is already in use.",
            "suggestions": [],
        }
    ...
    return {
        "available": not taken,
        "field": field,
        "message": "Phone number is available." if not taken else "That phone number is already in use.",
        "suggestions": [],
    }
...
def sync_oauth_profile(user, provider):
    ...
    payload = {
        ...
        "human_confirmed": bool((profile or {}).get("human_confirmed")),
        "terms_accepted": bool((profile or {}).get("terms_accepted")),
        "is_public": True,
...
def register_chain_user(email, password, username, full_name, extra=None):
    ...
    if not email or not password or not username or not full_name or not phone:
    ...
    if safe_select("chain_profiles", columns="id", filters={"phone": phone}, limit=1, order_by=None):
        return False, "Phone number is already registered."
        ...
        if extra:
            safe_update(
                "chain_profiles",
                {
                    ...
                    "account_mode": extra.get("profile_type"),
                    "dating_mode_enabled": bool(extra.get("dating_mode_enabled")),
                    "premium_mode_enabled": bool(extra.get("premium_mode_enabled")),
                    "signup_method": "email",
