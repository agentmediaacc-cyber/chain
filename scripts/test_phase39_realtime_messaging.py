...
def _fake_fast_query(sql_text, params=None, timeout_ms=2000, default=None):
    ...
    if "CHAIN_MESSAGE_REACTIONS" in sql_upper and "SELECT" in sql_upper:
        message_id = params[0] if params else None
        profile_id = params[1] if params and len(params) > 1 else None
        rows = [{"reaction_type": r["reaction"], "reaction": r["reaction"]} for r in _REACTIONS.get(message_id, []) if not profile_id or r["profile_id"] == profile_id]
        return rows
    if "CHAIN_MESSAGES" in sql_upper and "SELECT BODY" in sql_upper:
        msg = _MESSAGES.get(params[0] if params else None)
        return [{"body": msg.get("body")}] if msg else []
    if "CHAIN_MESSAGES" in sql_upper and "DELETED_FOR_EVERYONE" in sql_upper:
        msg = _MESSAGES.get(params[0] if params else None)
        return [{"deleted_for_everyone": bool(msg and msg.get("deleted_for_everyone"))}] if msg else []
    if "CHAIN_ONLINE_PRESENCE" in sql_upper and "SELECT" in sql_upper:
        pid = params[0] if params else None
        return [{"profile_id": pid, "status": _PRESENCE.get(pid, "offline")}]
    if "CHAIN_THREAD_MEMBERS" in sql_upper and "LAST_READ_AT" in sql_upper:
        return [{"last_read_at": "2026-01-01T00:00:00+00:00"}]
...
def _fake_send_message(thread_id, sender_profile_id, body, **kwargs):
    ...
    _MESSAGES[mid] = {"id": mid, "message_id": mid, "thread_id": thread_id, "sender_profile_id": sender_profile_id, "body": body, "deleted_for_everyone": False}
    return dict(_MESSAGES[mid])

def _fake_react_to_message(message_id, profile_id, reaction):
    _REACTIONS[message_id] = [r for r in _REACTIONS.get(message_id, []) if r["profile_id"] != profile_id]
    _REACTIONS.setdefault(message_id, []).append({"profile_id": profile_id, "reaction": reaction})
    return True

def _fake_remove_reaction(message_id, profile_id):
    _REACTIONS[message_id] = [r for r in _REACTIONS.get(message_id, []) if r["profile_id"] != profile_id]
    return True
...
def _fake_edit_message(message_id, profile_id, body):
    if message_id in _MESSAGES:
        _MESSAGES[message_id]["body"] = body
    return {"ok": True, "edited": True}

def _fake_delete_message_for_everyone(message_id, profile_id):
    if message_id in _MESSAGES:
        _MESSAGES[message_id]["deleted_for_everyone"] = True
    return {"ok": True}

def _fake_update_presence(profile_id, status="online", **kwargs):
    _PRESENCE[profile_id] = status
    return True

def _fake_get_presence(profile_id):
    return {"profile_id": profile_id, "status": _PRESENCE.get(profile_id, "offline")}
...
_mds_patch.remove_reaction = _fake_remove_reaction
_mds_patch.get_reactions = _fake_get_reactions
_mds_patch.edit_message = _fake_edit_message
_mds_patch.delete_message_for_everyone = _fake_delete_message_for_everyone
_mds_patch.update_presence = _fake_update_presence
_mds_patch.get_presence = _fake_get_presence
_mds_patch.unread_count = lambda profile_id: 0
_mds_patch.get_unread_counts_per_thread = lambda profile_id: {}
...
_mpr_patch.remove_reaction = _fake_remove_reaction
_mpr_patch.get_reactions = _fake_get_reactions
_mpr_patch.edit_message = _fake_edit_message
_mpr_patch.delete_message_for_everyone = _fake_delete_message_for_everyone
_mpr_patch.update_presence = _fake_update_presence
_mpr_patch.get_presence = _fake_get_presence
...
def setup():
    ...
    if os.getenv("CHAIN_TEST_FAKE_DB") == "1":
        ...
        MSG_ID = _fake_send_message(TID, PID_A, "Phase 39 test message").get("id")
        return
    for pid, uname, email in [
        (PID_A, "e2e_39_a", "e2e_39_a@test.chain"),
        (PID_B, "e2e_39_b", "e2e_39_b@test.chain")
    ]:
        for t in ["chain_call_participants", "chain_thread_members", "chain_online_presence", "chain_message_reactions"]:
